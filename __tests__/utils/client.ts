export const fetch_jsam_game_list = async (
...
/**
 * 囲碁将棋チャンネルの棋譜データを取得する
 *
 * 廃止済み: このエンドポイントは HTTP 500 / body 0 バイトしか返さない（2026-07-30 確認）。
 * 呼び出し側の `Parse Game > Loushou/Ginga` はコメントアウトしてあるため現在は未使用。
 * 対局一覧の `fetch_igoshogi_game_list` は健在。
 */
export const fetch_igoshogi_game = async ({ key }: { key: string }): Promise<Buffer> => {
...
export const fetch_meijin_game_list = async (): Promise<Buffer> => {
  const url: URL = new URL(
    '/list/meijin_all_game_list.txt',
    'https://d31j6ipzjd5eeo.cloudfront.net',
  )
  const response = await fetch(url.href, {
    headers: {
...
export const fetch_meijin_game = async (params: {
  key: string
  game_id?: number
}): Promise<Buffer> => {
  const { key } = params
